...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        for sentence in sents:
            ...
            for word in sentence:
                word_chars = [self.start_of_word ] + [self.char2id.get(c, self.char_unk) for c in word] + [self.end_of_word]
                sentence_chars.append(word_chars)
            results.append(sentence_chars)
        return results
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        ###     previous parts
        sents_chars = self.words2charindices(sents)
        padded_sents_chars = pad_sents_char(sents_chars, self.char2id['<pad>'])

        return torch.transpose(torch.tensor(padded_sents_chars, dtype=torch.long, device=device), 0, 1)
        ### END YOUR CODE
